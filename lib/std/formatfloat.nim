...
proc addCstringN(result: var string, buf: cstring; buflen: int) =
  ...
  when defined(nimNoLibc):
    var str = cast[ptr UncheckedArray[char]](buf)
    var i: int
    while i < buflen and str[i] != '\0':
      result[oldLen+i] = str[i]
  else:
...
proc writeFloatToBuffer*(buf: var array[65, char]; value: BiggestFloat | float32): int {.inline.} =
  when defined(nimPreviewFloatRoundtrip) or defined(nimPreviewSlimSystem) or defined(nimNoLic):
